// pages/city/[cityslug]/ClientPage.jsx
'use client';
...
export default function ClientPage({ cityslug }) {
  ...
  const fetcher = useCallback(async (url) => {
    ...
    if (!response.ok) {
      // If the backend returns 404, we catch it here and propagate the error.
      // This allows the error state to be handled.
  ...
  const { data, error, isLoading } = useSWR(`/api/city/${cityslug}?page=${page}`, fetcher, {
  ...
  const breadcrumb = useMemo(() => data?.breadcrumb || {}, [data]); // Extract breadcrumb data
  ...
  const handlePageClick = useCallback(
    (event) => {
      ...
      router.push(`/city/${cityslug}?page=${newPage}`, { shallow: true });
      window.scrollTo(0, 0);
    },
    [cityslug, pagination.page, router]
  );
  ...
  if (error) {
    ...
    // Display a more specific error message based on the backend's 404 response
    if (error.message.includes('404')) {
      return (
        <div className="text-center py-5">
          <h2 className="text-2xl font-bold mb-4">City Not Found</h2>
          <p>We couldn't find any data for "{formattedCity}". Please check the URL or try another city.</p>
  ...
  return (
    <>
          ...
          <div className="row">
            <div className="col-12">
              <div className="text-center">
                <h1 className="text-30 fw-600 text-white">Discover Luxe Stays In {formattedCity}</h1>
              </div>
            </div>
          </div>
        </div>
      ...
      <TopBreadCrumbCity breadcrumbData={breadcrumb} cityslug={cityslug} />
        ...
        <div className="container">
          <div className="row">
            {hotels.length > 0 ? (
              ...
              <div className="col-12 text-center">No hotels found for this city.</div>
            )}
          </div>
        </div>
      </section>
      ...
      <div className="pt-40 sm:pt-20 item_gap-x30">
        {relatedcity.length > 0 ? (
          <Relatedcity88 relatedcity={relatedcity} stateslug={breadcrumb.stateslug} countryslug={breadcrumb.countryslug} />
        ) : (
          ...
          <div className="pt-40 border-top-light">
            <div className="row y-gap-20">
              <div className="col-12 text-center">
                <h2 className="text-22 fw-500">FAQs about {formattedCity} hotels</h2>
              </div>
