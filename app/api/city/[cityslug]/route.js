...
export async function GET(req, { params }) {
  ...
  // Check cache first
  const cacheKey = `hotels_${cityslug}_page_${page}`;
  ...
  try {
    ...
    const query = `
      ...
      hotel_count AS (
        SELECT COUNT(*) AS total FROM public.hotels WHERE cityslug = $1
      )
      SELECT 
        hotel_data.*, 
        hotel_count.total
    ...
    `;
    const result = await client.query(query, [cityslug, LIMIT, offset]);

    if (result.rows.length === 0) {
      return new Response(JSON.stringify({ message: 'Tidak ada hotel ditemukan untuk cityslug ini' }), {
        status: 404,
      });
    }
    ...
    const relatedstateQuery = `
      SELECT DISTINCT city, cityslug, state, country 
      FROM public.hotels
      WHERE cityslug = $1
      AND city != '' 
      LIMIT 40
    `;
    const relatedstateResult = await client.query(relatedstateQuery, [cityslug]);
