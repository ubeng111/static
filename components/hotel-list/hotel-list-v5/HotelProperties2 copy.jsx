...
const HotelProperties2 = ({ hotels, cityName = "Lokasi Tidak Diketahui", dictionary }) => {
  ...
  if (!hotels || hotels.length === 0) {
    return <div>{commonDict.noHotelsFound || "Tidak ada hotel ditemukan untuk kota ini."}</div>; // Teks dari kamus
  }
  ...
  return (
    <>
      {hotels.map((item, index) => {
        const hotel = {
          id: item.hotelId,
          title: item.hotelName || commonDict.unnamedHotel || "Nama Hotel Tidak Tersedia", // Teks dari kamus
          img: item.imageURL || "/images/placeholder.jpg",
          slideimg: item.slideImages || [],
          location: cityName || commonDict.unknownLocation || "Lokasi Tidak Diketahui", // Teks dari kamus
              ...
              <div className="hotelsCard__content mt-10">
                ...
                <div className="d-flex align-items-center mt-20">
                  ...
                  <div className="text-14 text-dark-1 fw-bold ml-10">
                    {item.numberofreviews || 0} {commonDict.reviews || 'reviews'} {/* Teks dari kamus */}
                  </div>
                </div>
                <div className="mt-10">
                  <span className="text-16 fw-600 text-blue-1">
                    {currency.symbol} {hotel.dailyRate.toFixed(0)} / {commonDict.night || 'Night'} {/* Teks dari kamus */}
                  </span>
                </div>
              </div>
            </a>
