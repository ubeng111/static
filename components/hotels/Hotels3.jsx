...
const Hotels3 = ({ relatedHotels, itemsToShow = 4 }) => {
  // Check if relatedHotels data exists
  if (!relatedHotels || relatedHotels.length === 0) {
    return <div>No related hotels available.</div>;
  }

  // Only display the number of items specified by itemsToShow (default 4)
  const displayedHotels = relatedHotels.slice(0, itemsToShow);
    ...
    <>
      ...
      >
        {displayedHotels.map((item) => (
          <SwiperSlide key={item?.hotelslug}>
            <Link
