...
const Relatedcategory88 = React.memo(({ relatedcategory, categoryslug }) => {
  ...
  // Filter out countries without hotels
  const validCountries = relatedcategory?.filter((countryData) => countryData.hasHotels);

  if (!validCountries || validCountries.length === 0) {
    return (
      <div className="container">
        <h2 className="text-center fw-bold mb-3 text-dark">
          No Related Countries Found
        </h2>
      </div>
    );
  }
  ...
  return (
    <div className="container">
      ...
      <div className="row g-2">
        {validCountries.map((countryData, index) => {
